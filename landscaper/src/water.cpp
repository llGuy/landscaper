...
auto water_handler::prepare(glm::mat4 & view, glm::vec3 & cam_pos, glm::vec3 & light, f32 elapsed) -> void
{
	...
	dudv_texture.bind(GL_TEXTURE_2D, 1);
	normal_map_texture.bind(GL_TEXTURE_2D, 2);
}
...
auto water_handler::create_shaders(glm::mat4 & projection) -> void
{
	...
	water_shaders.get_uniform_locations("projection_matrix", "view_matrix", "camera_position", 
		"move_factor", "light_position", "reflection_texture", "dudv_map", "normal_map", "depth_texture");
	water_shaders.use();
